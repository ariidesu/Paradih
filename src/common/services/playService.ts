...
export function buildPlayService(app: FastifyInstance) {
    ...
    return {
        ...
        calculatePlayRating(score: number, chartConst: number): number {
            ...
            const rewards = [[900000, 3], [930000, 1], [950000, 1], [970000, 1], [980000, 1], [990000, 1]];

            let rating = 0;
            if (score >= 1009000) {
                rating = (chartConst * 10) + 7 + Math.pow((score - 1009000) / 1000, 1.35) * 3;
                ...
                rating += 10 * (chartConst * (Math.pow(score / 1000000, 1.5)) - 0.9);
            }
